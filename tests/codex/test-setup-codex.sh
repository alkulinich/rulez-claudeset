...
test_setup_codex_generates_global_agents_guidance() {
  ...
  assert_contains "<!-- generated by rulez-claudeset; do not edit directly -->" "$(cat "$agents")" "AGENTS.md is marked generated"
  ...
  assert_contains "Generated Codex guidance:" "$output" "setup-codex reports generated guidance"
}

test_setup_codex_rewrites_generated_agents_guidance() {
  local temp_home agents
  temp_home="$(make_temp_home)"
  agents="$temp_home/.codex/AGENTS.md"

  mkdir -p "$(dirname "$agents")"
  printf '<!-- generated by rulez-claudeset; do not edit directly -->\nstale\n' > "$agents"

  HOME="$temp_home" CODEX_DIR="$temp_home/.codex" bash "$REPO_ROOT/bin/setup-codex" >/dev/null

  assert_contains "Global rules applied to all projects via rulez-claudeset." "$(cat "$agents")" "setup-codex refreshes generated AGENTS.md"
  if [[ "$(cat "$agents")" == *"stale"* ]]; then
    TESTS_RUN=$((TESTS_RUN + 1))
    TESTS_FAILED=$((TESTS_FAILED + 1))
    printf '  FAIL: setup-codex should replace stale generated AGENTS.md content\n'
  else
    TESTS_RUN=$((TESTS_RUN + 1))
    printf '  ok: setup-codex replaces stale generated AGENTS.md content\n'
  fi
}

test_setup_codex_refuses_to_overwrite_manual_agents_guidance() {
  ...
  assert_eq "1" "$status" "setup-codex fails when AGENTS.md is manually managed"
  assert_contains "Refusing to overwrite non-generated Codex guidance" "$output" "setup-codex explains manual AGENTS.md collision"
  assert_eq "# My Codex rules" "$(cat "$agents")" "setup-codex preserves manual AGENTS.md"
}
